...
الگوی طراحی state - مسیرها می‌توانند به طور کامل یا یک‌طرفه باشند یا دوطرفه و بر اساس اینکه در کدام یکی از این دو حالت قرار گرفته باشند، کاربر پاسخ متفاوتی به سوال خود می‌گیرد چون رفتار کلاس مورد نظر تغییر می‌کند. در فاز اول هم برای تحقق این الگو ابتدا یک اینترفیس RouteState را تعریف می‌کنیم که شامل دو تابع برای یک‌طرفه یا دوطرفه کردن مسیر است. سپس برای هر کدام از حالات کلاسی را تعریف می‌کنیم که این اینترفیس را  پیاده‌سازی کرده است. پیاده‌سازی تابع makeBidirectional در کلاس BidirectionalState به این صورت است که تمام یال‌هایی که یک‌طرفه (directed = true) هستند را دوطرفه می‌کند ولی پیاده‌سازی این تابع در کلاس UnidirectionalState که یعنی در حالت مسیر یک‌طرفه هستیم، حالت گراف را روی دوطرفه ست می‌کند. پیاده‌سازی تابع makeUnidirectional در کلاس UnidirectionalState به اینگونه است که بدنه‌ی خالی دارد چون استیت قبلی‌مان یک‌طرفه بوده و با ترنزیشن به استیت یک‌طرفه نیازی به انجام کاری نیست. پیاده‌سازی این تابع در کلاس BidirectioanlState به اینگونه است که حالت گراف را روی دوطرفه ست می‌کند چون از حالت یک‌طرفه می‌خواهیم به حالت دوطرفه ترنزیشن داشته باشیم.
